...
Se mettiamo una funzione come dipendenza all'interno di useEffect, e questa
funzione è implementata all'interno del componente, l'effetto verrà chiamato ad
ogni render, quindi si ha un loop infinito di rendering. UseCallback risolve a
questo problema, se la funzione non cambia, allora non si avrà altri render non
necessari. UseCallback è uno shortcut di useMemo, ma per le funzioni.
